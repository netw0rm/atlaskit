...
storiesOf(name, module)
  .add('a simple ak-badge', () => (
    <Component />
  ))
  .add('an ak-badge that emits an action when it is clicked', () => (
    <Component id="myComponent" onClick={action('clicking the WebComponent')} />
  ))
  .add('an ak-badge that removes itself when being clicked', () => {
  ...
  .addMonkeyTest('a ak-badge with monkey testing', () => (
    // Use this to add a story that has fuzzy testing attached.
    <Component />
  ))
  .addMonitored('an ak-badge with monitored performance', () => (
