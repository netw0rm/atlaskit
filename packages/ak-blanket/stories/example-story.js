...
storiesOf(name, module)
  .add('a simple ak-blanket', () => (
    <Component />
  ))
  .add('a simple ak-blanket with a name', () => (
    <Component name="MyComponent" />
  ))
  .add('an ak-blanket that emits an action when it is clicked', () => (
    <Component id="myComponent" onClick={action('clicking the WebComponent')} />
  ))
  .add('an ak-blanket that removes itself when being clicked', () => {
  ...
  .addMonkeyTest('a ak-blanket with monkey testing', () => (
    // Use this to add a story that has fuzzy testing attached.
    <Component />
  ))
  .addMonitored('an ak-blanket with monitored performance', () => (
