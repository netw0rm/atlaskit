...
describe('ak-inline-dialog', () => {
  ...
  describe('default', () => {
    ...
    it('should render any children passed to it', () => {
      ...
      expect(wrapper.find('#children').length.length).not.to.equal(0);
    });
  });
  ...
  describe('isOpen prop', () => {
    it('should render the content container if isOpen is set', () => {
      ...
      expect(wrapper.find(`.${containerClass}`).length.length).not.to.equal(0);
    });
  ...
  describe('content prop', () => {
    ...
    it('should render content if isOpen is set', () => {
      ...
      expect(wrapper.find('#someContent').length.length).not.to.equal(0);
    });
    ...
    it('should reflect content prop onto Layer if isOpen is set', () => {
      ...
      expect(foo.find('#someContent').length.length).not.to.equal(0);
    });
  });
