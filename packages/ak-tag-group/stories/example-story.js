...
storiesOf(name, module)
  ...
  .add('a simple ak-tag-group with a name', () => (
    <Component name="MyComponent" />
  ))
  .add('an ak-tag-group that emits an action when it is clicked', () => (
    <Component id="myComponent" onClick={action('clicking the WebComponent')} />
  ))
  .add('an ak-tag-group that removes itself when being clicked', () => {
  ...
  .addMonkeyTest('a ak-tag-group with monkey testing', () => (
    // Use this to add a story that has fuzzy testing attached.
    <Component />
  ))
  .addMonitored('an ak-tag-group with monitored performance', () => (
