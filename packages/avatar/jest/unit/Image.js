...
describe('Avatar', () =>
  describe('Image', () => {
    ...
    it('should not render an img when the src is not set"', () =>
      expect(shallow(<Image />).find('img').exists()).toBe(false)
    );

    describe('default avatar', () => {
      describe('should render default avatar', () => {
        it('when no properties are provided', () =>
          expect(shallow(<Image />).find(DefaultImage).exists()).toBe(true)
        );

        it('when there is an error', () =>
          expect(shallow(<Image hasError />).find(DefaultImage).exists()).toBe(true)
        );

        it('when src is set and there is an error', () =>
          expect(shallow(<Image src={src} hasError />).find(DefaultImage).exists()).toBe(true)
        );
      });

      describe('should not render default avatar', () => {
        it('when loading=true and no src', () =>
          expect(shallow(<Image isLoading />).find(DefaultImage).exists()).toBe(false)
        );

        it('when src is set', () =>
          expect(shallow(<Image src={src} />).find(DefaultImage).exists()).toBe(false)
        );
      });
    });
  })
