...
describe('getPropertyAppearance', () => {
  ...
  it('should use the default appearance if the appearance is not defined.', () =>
    expect(getPropertyAppearance('propertyA', { theme: 'themeB', appearance: 'not-an-appearance' }, themeDefinitions)).toBe(theme.default.propertyA.default)
  );

  it('should return the \'hover\' value when in the hover state.', () =>
    expect(getPropertyAppearance('propertyA', { isHover: true }, themeDefinitions)).toBe(theme.default.propertyA.hover)
  );

  it('should prioritise \'active\' state over \'hover\' state.', () =>
    expect(getPropertyAppearance('propertyA', { isActive: true, isHover: true }, themeDefinitions)).toBe(theme.default.propertyA.active)
  );

  it('should prioritise \'selected\' state over \'active\' and \'hover\' states.', () =>
    expect(getPropertyAppearance('propertyA', { isActive: true, isHover: true, isSelected: true }, themeDefinitions)).toBe(theme.default.propertyA.selected)
  );

  it('should prioritise \'disabled\' state over all other states.', () =>
    expect(getPropertyAppearance('propertyA', { disabled: true, isActive: true, isHover: true, isSelected: true }, themeDefinitions)).toBe(theme.default.propertyA.disabled)
  );

  it('should use the default value if the state value is not defined.', () =>
    expect(getPropertyAppearance('propertyA', { appearance: 'appearanceB', isSelected: true }, themeDefinitions)).toBe(theme.appearanceB.propertyA.default)
  );
});
