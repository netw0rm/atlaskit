...
import { KEY_SPACE, KEY_ENTER, KEY_DOWN } from '../../src/util/keys';
...
describe(name, () => {
  ...
  describe('render', () => {
    ...
    test('should pass required properties to the button trigger', () => {
      [
        <Menu triggerType="button" trigger="text">{itemsList}</Menu>,
        <Menu triggerType="button" defaultOpen trigger="text">{itemsList}</Menu>,
      ].forEach((val) => {
    ...
    test('should default to button with expand icon for triggerType=button with no overrides', () => {
      const text = 'text';
      const menu = mount(<Menu triggerType="button" trigger={text}>{itemsList}</Menu>);
    ...
    test('should pass through triggerButtonProps to the trigger for triggerType=button', () => {
      ...
      const menu = mount(<Menu triggerType="button" triggerButtonProps={triggerProps}>{itemsList}</Menu>);
      const trigger = menu.find(Button);
  ...
  describe('show/hide logic', () => {
    ...
    test('clicking with trigger should open the dropdown', () => {
      const wrapper = mount(<Menu trigger={<div id="trigger">test</div>} />);
