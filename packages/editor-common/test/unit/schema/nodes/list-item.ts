...
import { toHTML, fromHTML, doc, ol, li, p, schema } from '../../../helpers';
...
describe(`${name}/schema listItem node`, () => {
  ...
  it('should be possible to have sublist inside list', () => {
    ...
    expect(docFromHTML.toJSON()).toEqual(doc(ol(li(p('text'), ol(li(p('sublist')))))).toJSON());
  });
  ...
  it('should not be possible to have sublist and first child inside list', () => {
    ...
    expect(docFromHTML.toJSON()).toEqual(doc(ol(li(p('sublist'), p('text')))).toJSON());
  });
});
