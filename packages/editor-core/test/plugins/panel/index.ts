...
import { doc, panel, p, makeEditor, fixtures, createEvent, panelText } from '../../../src/test-helper';
...
describe('@atlaskit/editor-core ui/PanelPlugin', () => {
  ...
  describe('API', () => {
    it('should allow a change handler to be registered', () => {
      const { pluginState } = editor(doc(panel(panelText(p('te{<>}xt')))));
    ...
    it('should call subscribers when panel is clicked', () => {
      const { editorView, plugin, pluginState, sel } = editor(doc(panel(panelText(p('te{<>}xt')))));
      const spy = sinon.spy();
      pluginState.subscribe(spy);
    ...
    it('should not call subscribers when another block in editor is clicked', () => {
      const { editorView, plugin, pluginState, sel } = editor(doc(p('te{<>}xt'), panel(panelText(p('text')))));
      const spy = sinon.spy();
      pluginState.subscribe(spy);
      plugin.props.handleClick!(editorView, sel, event);
      expect(spy.callCount).to.equal(1);
    ...
    it('should call subscribers when panel was focused and editor blur', () => {
      const { editorView, plugin, pluginState } = editor(doc(panel(panelText(p('te{<>}xt')))));
      const spy = sinon.spy();
      pluginState.subscribe(spy);
    ...
    it('should not call subscribers when another block was focused and editor blur', () => {
      const { editorView, plugin, pluginState } = editor(doc(p('te{<>}xt'), panel(panelText(p('text')))));
      const spy = sinon.spy();
      pluginState.subscribe(spy);
      plugin.props.onFocus!(editorView, event);
      plugin.props.onBlur!(editorView, event);
    ...
    it('should not call subscribers when panel received focus', () => {
      const { editorView, plugin, pluginState } = editor(doc(panel(panelText(p('text')))));
      const spy = sinon.spy();
      pluginState.subscribe(spy);
      plugin.props.onFocus!(editorView, event);
      expect(spy.callCount).to.equal(1);
    ...
    it('should be able to change panel type using function changeType', () => {
      const { pluginState, editorView } = editor(doc(panel(panelText(p('te{<>}xt')))));
