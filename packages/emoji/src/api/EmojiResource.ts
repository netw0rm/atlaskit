...
export interface EmojiProvider extends Provider<string, EmojiSearchResult, any, undefined, SearchOptions> {
  ...
  /**
   * Load emoji image that may require authentication to download.
   *
   ...
   */
  loadEmojiImageData(emoji: EmojiDescription): OptionalEmojiDescription | Promise<OptionalEmojiDescription>;

  /**
   * Returns true if a media emoji should be rendered optimistically,
   ...
   */
  optimisticEmojiRendering(emoji: EmojiDescription): boolean;
}
...
export class EmojiResource extends AbstractResource<string, EmojiSearchResult, any, undefined, SearchOptions> implements EmojiProvider {
  ...
  optimisticEmojiRendering(emoji: EmojiDescription): boolean {
    ...
    if (isPromise(optimistic)) {
      // Not sure yet, so lets say no for now (this should be primed in most/all cases)
      return false;
    }
