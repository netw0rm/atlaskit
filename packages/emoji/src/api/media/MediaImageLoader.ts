...
const defaultConcurrentDownloadLimit = 8;
...
export default class MediaImageLoader {
  ...
  private processFromQueue() {
    while (this.activeProcessing < this.concurrentDownloadLimit && this.mediaImageQueue.length > 0) {
      ...
      this.tokenManager.getToken('read', false)
        .then(token => {
          this.requestMediaEmoji(url, token, true).then(dataURL => {
            resolve(dataURL);
            this.completedItem();
          }).catch(error => {
            reject(error);
  ...
  private requestMediaEmoji(url: string, token: MediaApiToken, retryOnAuthError: boolean): Promise<DataURL> {
    return imageAcceptHeader().then(acceptHeader => {
      ...
      return fetch(new Request(url, options)).then(response => {
        if (response.status === 403 && retryOnAuthError) {
          // retry once if 403
          return this.tokenManager.getToken('read', true)
