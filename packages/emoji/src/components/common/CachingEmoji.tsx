...
export class CachingMediaEmoji extends PureComponent<EmojiProps,State> {
  ...
  private loadEmoji(emoji: EmojiDescription, context: EmojiContext, forceLoad: boolean): EmojiDescription | undefined {
    ...
    const { emojiProvider, onEmojiLoaded } = context.emoji;
    if (!emojiProvider) {
      return undefined;
    }

    const optimisticRendering = emojiProvider.optimisticEmojiRendering(emoji);
    ...
    const loadedEmoji = emojiProvider.loadEmojiImageData(emoji);
