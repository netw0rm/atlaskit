...
import { createPngFile, newEmojiRepository, getEmojiResourcePromise, getNonUploadingEmojiResourcePromise, pngDataURL, pngFileUploadData } from '../../TestData';
...
function setupPicker(props?: Props): Promise<ReactWrapper<any, any>> {
  ...
  return waitUntil(() => picker.find(EmojiPickerComponent).length > 0)
    .then(() => picker);
}
...
describe('<EmojiPicker />', () => {
  ...
  describe('upload', () => {
    ...
    it('Upload main flow interaction', () => {
      ...
      return setupPicker({ emojiProvider, hideToneSelector: true }).then(component =>
