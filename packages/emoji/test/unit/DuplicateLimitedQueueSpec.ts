...
describe('DuplicateLimitedQueue', () => {
  describe('initialisation', () => {
    it('should throw for illegal constructor parameters', () => {
      expect(() => new DuplicateLimitedQueue(0, 2)).to.throw(RangeError);
      expect(() => new DuplicateLimitedQueue(3, 0)).to.throw(RangeError);
    });

    it('should behave correctly when empty', () => {
      const queue = new DuplicateLimitedQueue(10, 5);
  ...
  describe('limits', () => {
    it('should respect maximum size for unique set of items', () => {
      ...
      const queue = new DuplicateLimitedQueue(3,2); /// queue size of 6
    ...
    it('should respect maximum size of one', () => {
      const queue = new DuplicateLimitedQueue(1,1); /// queue size of 1
      queue.enqueue('apple');
    ...
    it('should respect maximum duplicate size', () => {
      ...
      const queue = new DuplicateLimitedQueue(3, 2);
  ...
  describe('ordering', () => {
    it('should order most duplicated items first', () => {
      ...
      const queue = new DuplicateLimitedQueue(3, 3);
      itemsToAdd.map(value => queue.enqueue(value));
      ...
      equalFrequencyItems = equalFrequencyItems.filter(value => value !== queuedItems[2]);
    ...
    it('should re-order when some item becomes more duplicated', () => {
      ...
      const queue = new DuplicateLimitedQueue(4, 3);
      itemsToAdd.map(value => queue.enqueue(value));
