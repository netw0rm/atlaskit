...
import { UploadPromptMessage, addEmojiClassName } from '../../src/components/picker/EmojiPickerUploadPrompts';
...
export function setupPickerWithoutToneSelector(): Promise<ReactWrapper<any, any>> {
  return setupPicker({
...
export function setupPicker(
  ...
  return waitUntil(() => picker.update() && picker.find(EmojiPickerComponent).length > 0).then(
    () => picker,
  );
}
...
// Component should be mounted picker
export const emojisVisible = (component, list) => hasSelector(component, Emoji, list);
...
export const categoryVisible = (category: string, component) => findCategoryHeading(category, component).length > 0;
...
export const showCategory = (category: string, component): Promise<any> => {
  ...
  return waitUntil(() => emojisVisible(component, list)).then(() => {
    ...
    return waitUntil(() => component.update() && categoryVisible(category, component.find(EmojiPickerList)));
  });
};
...
export const searchInputVisible = component => findSearchInput(component).length > 0;

const findCustomSection = component =>
  component.findWhere(wrapper =>
...
export const findEmojiNameInput = component =>
  component.update() && component.find(`.${commonStyles.uploadChooseFileEmojiName} input`);
...
export const findPreview = component => component.update() && component.find(EmojiPreview);

export const previewVisible = component => findPreview(component).length > 0;

export const findUploadError = component => component.update() && component.find(`.${commonStyles.uploadError}`);

export const uploadErrorVisible = component => findUploadError(component).length > 0;
