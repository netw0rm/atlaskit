...
describe('<EmojiPicker />', () => {
  ...
  describe('display', () => {
    ...
    it('should tone selector in preview by default', () => {
      ...
      expect(previewEmoji.at(0).prop('emoji').shortName).toEqual(':raised_hand:');
    });
  ...
  describe('selection', () => {
    ...
    it('selecting emoji should call recordSelection on EmojiProvider', async () => {
      ...
      expect(provider.recordedSelections[0].shortName).toEqual(helper.allEmojis[clickOffset].shortName);
    });
  });
  ...
  describe('upload', () => {
    ...
    it('Upload main flow interaction', async () => {
      ...
      expect(helper.findEmojiNameInput(component).prop('value')).toEqual('cheese_burger');
      ...
      await waitUntil(() => helper.emojiWithIdVisible(component, upload.emoji.id));

      // new emoji in view
      const newEmojiDescription = provider.getUploads()[0].emoji;
      emoji = helper.findEmojiWithId(
        component,
    ...
    it('Upload cancel interaction', async () => {
      ...
      const previewEmoji = preview.find(Emoji);
      expect(previewEmoji).toHaveLength(1);
      expect(previewEmoji.prop('emoji').shortName).toEqual(initalPreviewEmoji.prop('emoji').shortName);
