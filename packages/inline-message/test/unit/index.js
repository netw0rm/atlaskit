    ...
    describe('props', () => {
      ...
      describe('type', () => {
        it('connectivity type produces connectivity icon', () => {
          const wrapper = shallow(<IconForType isHovered={false} isOpen={false} type="connectivity" />);
        ...
        it('confirmation type produces confirmation icon', () => {
          const wrapper = shallow(<IconForType isHovered={false} isOpen={false} type="confirmation" />);
        ...
        it('info type produces info icon', () => {
          const wrapper = shallow(<IconForType isHovered={false} isOpen={false} type="info" />);
          expect(wrapper.find(WarningIcon).length).toBeGreaterThan(0);
          expect(wrapper.find(IconWrapper).prop('appearance')).toBe('info');
        });
        it('warning type produces warning icon', () => {
          const wrapper = shallow(<IconForType isHovered={false} isOpen={false} type="warning" />);
          expect(wrapper.find(WarningIcon).length).toBeGreaterThan(0);
          expect(wrapper.find(IconWrapper).prop('appearance')).toBe('warning');
        });
        it('error type produces error icon', () => {
          const wrapper = shallow(<IconForType isHovered={false} isOpen={false} type="error" />);
          expect(wrapper.find(WarningIcon).length).toBeGreaterThan(0);
