...
import { Actions, ActionsMenu, ActionButtonWrapper, FailureMessageBlock, FailureMessage, SuccessMessage } from '../styled/ActionsView';
...
export class ActionsView extends React.Component<ActionsViewProps, ActionsViewState> {
  ...
  handleActionButton = (action: ActionsViewAction) => (event: Event) => {
    ...
    if (onActionClick) {
      onActionClick(action.appCardAction, this.createCallbackHandlers(action.key));
    }
  }
  ...
  createCallbackHandlers(actionType: number): AppCardActionCallbackHandlers {
    return {
      progress: () => this.updateProgressState({
        actionInProgress: actionType
      }),
      success: (message) => this.updateProgressState({
        actionInProgress: null,
        successMessage: message || null,
      }),
      failure: (message, tryAgain = false, tryAgainLinkText = TRY_AGAIN_LINK_TEXT) => this.updateProgressState({
        actionInProgress: null,
  ...
  get isOtherActionInProgress(): boolean {
    ...
    return this.otherActions.some(action => action.key === this.state.actionInProgress);
  }
  ...
  getActionButton(action: ActionsViewAction | undefined) {
    if (action) {
      ...
      const spinner = isCurrentActionInProgress ? <Spinner size="small" invertColor={isInversed} /> : null;
      const content = isCurrentActionInProgress ? null : action.appCardAction.title;
      return (
        <ActionButtonWrapper>
          <UntypedButton isDisabled={this.isInProgress} onClick={this.handleActionButton(action)} theme={this.theme} iconAfter={spinner} >
            {content}
          </UntypedButton>
        </ActionButtonWrapper>
      );
  ...
  getMeatballsButton() {
    ...
    return <UntypedButton isDisabled={this.isInProgress} appearance="subtle" iconBefore={icon} theme={this.theme} />;
  }
  ...
  render(): JSX.Element {
    ...
    const { failureMessage, successMessage, tryAgain, tryAgainLinkText } = this.state;

    if (failureMessage !== null) {
      return (
        <FailureMessageBlock>
          <FailureMessage tryAgain={tryAgain}>{failureMessage}</FailureMessage>
          {tryAgain ? <UntypedButton appearance="link" onClick={this.handleTryAgain} theme={this.theme}>{tryAgainLinkText}</UntypedButton> : null}
        </FailureMessageBlock>
      );
    }
