...
import { ActionsMenu, SuccessMessage, FailureMessage } from '../../../../src/app/styled/ActionsView';
...
describe('ActionsView', () => {
  ...
  it('should pass action and progress handlers to callback on button click', () => {
    ...
    const createCallbackHandlersSpy = jest.spyOn(ActionsView.prototype, 'createCallbackHandlers');
    const actions: AppCardAction[] = [{
      title: 'Open',
      target: {
        receiver: 'some.app',
    ...
    const element = mount(<ActionsView actions={actions} isInversed={false} onActionClick={onActionClickSpy}/>);
  ...
  it('should update state on progress handler execution', () => {
    ...
    const onActionClickSpy = jest.fn().mockImplementation((action, handlers) => {
      handlers.progress();
    });
    const createCallbackHandlersSpy = jest.spyOn(ActionsView.prototype, 'createCallbackHandlers');
    const actions: AppCardAction[] = [{
    ...
    const element = mount(<ActionsView actions={actions} isInversed={false} onActionClick={onActionClickSpy}/>);
    element.find(Button).at(1).simulate('click', event);
    ...
    expect(element.state()).toEqual(expect.objectContaining({
      actionInProgress: 1
    }));
  });
  ...
  it('should update state on success handler execution', () => {
    ...
    const element = mount(<ActionsView actions={actions} isInversed={false} onActionClick={onActionClickSpy}/>);
    element.find(Button).at(0).simulate('click', event);
    expect(onActionClickSpy).toHaveBeenCalledWith(actions[0], {
      failure: expect.any(Function),
      success: expect.any(Function),
      progress: expect.any(Function)
  ...
  it('should update state on failure handler execution', () => {
    ...
    const element = mount(<ActionsView actions={actions} isInversed={false} onActionClick={onActionClickSpy}/>);
    element.find(DropdownMenu).prop('onItemActivated')!({ item: { action: { appCardAction: actions[2], key: 3 } } });
