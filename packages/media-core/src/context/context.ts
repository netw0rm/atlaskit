...
class ContextImpl implements Context {
  ...
  constructor(readonly config: ContextConfig) {
    this.fileItemCache = new LRUCache<string, FileItem>(config.cacheSize || DEFAULT_CACHE_SIZE);
  }

  getMediaItemProvider(id: string,
                       mediaItemType: MediaItemType,
                       collectionName?: string,
                       mediaItem?: MediaItem): MediaItemProvider {
    const isMediaItemLink = mediaItem && mediaItem.type === 'link';
    const isMediaItemFileAndNotPending = mediaItem
      && mediaItem.type === 'file'
  ...
  getMediaCollectionProvider(collectionName: string,
                             pageSize: number = DEFAULT_COLLECTION_PAGE_SIZE): MediaCollectionProvider {
  ...
  addLinkItem(url: string, collectionName: string, metadata?: UrlPreview): Promise<string> {
    ...
    return linkService.addLinkItem(
      url,
      collectionName,
      metadata
    );
