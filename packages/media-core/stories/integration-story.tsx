...
const playgroundBaseURL = 'https://media-playground.internal.app.dev.atlassian.io';
const access = {
...
const requestToken = ({access, withAsapIssuer, collectionName}) => {
  return axios.post('/token', {access}, {
...
const clientIdBaseAuthProvider = (collection): Promise<Auth> =>
  requestToken({access, collectionName, withAsapIssuer: false}).then(tokenDataToAuth);

const asapIssuerBaseAuthProvider = (collection): Promise<Auth> =>
  requestToken({access, collectionName, withAsapIssuer: true}).then(tokenDataToAuth);
...
class AuthPlayground extends Component<Props, State> {
  ...
  getMediaCollection = () => {
    const mediaCollectionProvider = this.context.getMediaCollectionProvider(collectionName, 3);
  ...
  renderCollectionItems = () =>
    this.state.items.map((item: MediaItem) => {
      ...
      if (item.type === 'file' && this.state.auth) {
        ...
        return <li key={itemId}><img src={fileThumbnailUrl}/></li>;
      } else {
        return <li key={itemId}>{item.type} - ${itemId}</li>;
      }
    })
    ...
    if (auth) {
      if (isClientBasedAuth(auth)) {
        return <FormattedBlock>
          <strong>Client Id:</strong> {auth.clientId}<br/>
          <strong>Token:</strong> {auth.token}
        </FormattedBlock>;
      } else {
        return <FormattedBlock>
          <strong>Asap Issuer:</strong> {auth.asapIssuer}<br/>
          <strong>Token:</strong> {auth.token}
        </FormattedBlock>;
      }
    }
  ...
  render() {
    return <div style={{padding: 10}}>
      ...
      <br/>
      <Button appearance="primary" onClick={this.getMediaCollection}>Get media collection</Button>
      <br/>
...
storiesOf('Integration', {})
  .add('with clientId', () => {
    return <AuthPlayground authProvider={clientIdBaseAuthProvider}/>;
  })
  .add('with asapIssuer', () => {
    return <AuthPlayground authProvider={asapIssuerBaseAuthProvider}/>;
  });
