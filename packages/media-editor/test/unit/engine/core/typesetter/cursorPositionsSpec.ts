...
describe('MediaEditor cursor positions', () => {
  ...
  describe('getDirection', () => {
    ...
    it('should return ltr if previous character is to the left and close', () => {
      jest.spyOn(first, getBoundingClientRectMethod).mockImplementation(() => ({left: 10, right: 12.3}));
      jest.spyOn(second, getBoundingClientRectMethod).mockImplementation(() => ({left: 12.4, right: 20}));

      expect(getDirection(1, [first, second])).toBe('ltr');
    });

    it('should return undefined if previous character is to the left but far', () => {
      jest.spyOn(first, getBoundingClientRectMethod).mockImplementation(() => ({left: 10, right: 11.3}));
      jest.spyOn(second, getBoundingClientRectMethod).mockImplementation(() => ({left: 12.4, right: 20}));
    ...
    it('should return rtl if previous character is to the right and close', () => {
      jest.spyOn(first, getBoundingClientRectMethod).mockImplementation(() => ({left: 30, right: 32.3}));
      jest.spyOn(second, getBoundingClientRectMethod).mockImplementation(() => ({left: 21.5, right: 29.4}));
    ...
    it('should return undefined if previous character is to the right but far', () => {
      jest.spyOn(first, getBoundingClientRectMethod).mockImplementation(() => ({left: 30, right: 32.3}));
      jest.spyOn(second, getBoundingClientRectMethod).mockImplementation(() => ({left: 21.5, right: 23.4}));
  ...
  describe('putInternalPositions', () => {
    ...
    it('should add multiple positions if the the text contains multiple characters', () => {
      ...
      jest.spyOn(span, getBoundingClientRectMethod).mockImplementation(() => {
        callCount++;
        if (callCount === 1) { return {left: 10, right: 20}; }
        if (callCount === 2) { return {left: 10, right: 30}; }
        if (callCount === 3) { return {left: 10, right: 40}; }
        if (callCount === 4) { return {left: 10, right: 50}; }
        if (callCount === 5) { return {left: 10, right: 60}; }
      });
