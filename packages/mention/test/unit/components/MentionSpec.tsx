...
describe('<Mention />', () => {
  describe('Mention', () => {
    ...
    it('should render a default lozenge if no accessLevel data and is not being mentioned', () => {
      const mention = mount(<Mention {...mentionData} />);
      expect(mention.find(MentionStyle).prop('mentionType')).toEqual(MentionType.DEFAULT);
    });
    ...
    it('should add a highlighted lozenge if `isHighlighted` is set to true', () => {
      ...
      expect(mention.find(MentionStyle).prop('mentionType')).toEqual(MentionType.SELF);
    });
    ...
    it('should render a restricted style lozenge if the user has non-CONTAINER permissions', () => {
      ...
      expect(mention.find(MentionStyle).prop('mentionType')).toEqual(MentionType.RESTRICTED);
    });
    ...
    it('should dispatch lozenge.select analytics onClick-event', () => {
      ...
      expect(analyticsSpy).toHaveBeenCalledWith('atlassian.fabric.mention.lozenge.select', {accessLevel: 'CONTAINER', isSpecial: false});
    });
    ...
    it('should dispatch lozenge.hover analytics onMouseLeave-event', () => {
      ...
      expect(analyticsSpy).toHaveBeenCalledWith('atlassian.fabric.mention.lozenge.hover', {accessLevel: 'CONTAINER', isSpecial: false});
    });

    it('should render a stateless mention component with correct data attributes', () => {
     const mention = mount(<Mention {...mentionData} accessLevel="NONE" />);
     expect(mention.getDOMNode().attributes.getNamedItem('data-mention-id').value).toEqual(mentionData.id);
     expect(mention.getDOMNode().attributes.getNamedItem('data-access-level').value).toEqual('NONE');
    });
  });
  ...
  describe('ResourcedMention', () => {
    ...
    it('should render a highlighted statless mention component if mentionProvider.shouldHighlightMention returns true', async () => {
      ...
      expect(mention.find(Mention).first().find(MentionStyle).prop('mentionType')).toEqual(MentionType.SELF);
    });
    ...
    it('should not render highlighted mention component if there is no mentionProvider', () => {
      ...
      expect(mention.find(Mention).first().find(MentionStyle).prop('mentionType')).toEqual(MentionType.DEFAULT);
    });
