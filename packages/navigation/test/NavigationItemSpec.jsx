...
describe('<NavigationItem />', () => {
  describe('props', () => {
    it('icon should render an image', () => {
      expect(mountWithTheme(<NavigationItem icon={<img alt="foo" />} />).find('img').length).to.equal(1);
    });
    it('isSelected=true should render a child with the isSelected prop', () => {
      expect((mountWithTheme(<NavigationItem isSelected />).find('NavigationItemOuter')).props().isSelected).to.equal(true);
    });
    it('isSelected=false should not render a child with the isSelected prop', () => {
      expect(mountWithTheme(<NavigationItem />).find('NavigationItemOuter').props().isSelected).to.equal(false);
    });
    it('with a href should render onto the link', () => {
      expect(mountWithTheme(<NavigationItem href="foo" />).find('InteractiveWrapper').props().href).to.equal('foo');
    });
    it('with no href should not render a link', () => {
      expect(mountWithTheme(<NavigationItem />).find('a').length).to.equal(0);
    });
    it('with an onClick should call the onClick', () => {
      const spy = sinon.spy();
      const navigation = mountWithTheme(<NavigationItem onClick={spy} />);
    ...
    it('with an onClick and href should render the href on a link, and bind the onClick to it', () => {
      const spy = sinon.spy();
      const navigation = mountWithTheme(<NavigationItem href="foo" onClick={spy} />);
    ...
    it('linkComponent should render a custom link component', () => {
      const customLink = mountWithTheme(
        <NavigationItem
          href="#custom-href"
    ...
    it('textAfter should render in the navigation item', () => {
      expect(mountWithTheme(<NavigationItem action={<span className="ACTION" />} />).find('.ACTION')).to.have.length.above(0);
    });
    it('action should render in the navigation item', () => {
      expect(mountWithTheme(<NavigationItem textAfter={<span className="TEXTAFTER" />} />).find('.TEXTAFTER')).to.have.length.above(0);
    });
    it('textAfter should not render if the prop is not set', () => {
      expect(mountWithTheme(<NavigationItem />).find('TextAfter').length).to.equal(0);
    });
    it('action should not render if the prop is not set', () => {
      expect(mountWithTheme(<NavigationItem />).find('NavigationItemAction').length).to.equal(0);
    });
    ...
    it('subText should render in the navigation item', () => {
      expect(mountWithTheme(<NavigationItem subText="SUBTEXT" />).html()).to.contain('SUBTEXT');
    });

    describe('isDropdownTrigger=true and dropIcon is provided', () => {
      it('should render dropIcon', () => {
        const wrapper = mountWithTheme(<NavigationItem isDropdownTrigger dropIcon={<img alt="foo" />} />);
      ...
      describe('if textAfter is provided', () => {
        it('should render NavigationItemAfter wrapper with isDropdownTrigger prop forwarded', () => {
          const wrapper = mountWithTheme(<NavigationItem isDropdownTrigger dropIcon={<img alt="foo" />} textAfter="test" />);
    ...
    describe('isDropdownTrigger=false and dropIcon is provided', () => {
      it('should not render dropIcon', () => {
        const wrapper = mountWithTheme(<NavigationItem dropIcon={<img alt="foo" />} />);
      ...
      describe('if textAfter is provided', () => {
        it('should render NavigationItemAfter wrapper without isDropdownTrigger prop forwarded', () => {
          const wrapper = mountWithTheme(<NavigationItem dropIcon={<img alt="foo" />} textAfter="test" />);
  ...
  describe('behaviour', () => {
    it('mousedown on the link is prevented', () => {
      const spy = sinon.spy();
      mountWithTheme(<NavigationItem href="foo" />).find('InteractiveWrapper').simulate('mouseDown', {
        preventDefault: spy,
      });
