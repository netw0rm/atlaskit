...
import NavigationItemGroupTitle from '../../src/components/styled/NavigationItemGroupTitle';
import NavigationItemGroupSeparator from '../../src/components/styled/NavigationItemGroupSeparator';
import NavigationItemGroupAction from '../../src/components/styled/NavigationItemGroupAction';
...
describe('<NavigationItemGroup />', () => {
  describe('props', () => {
    ...
    it('separator should render in the container item group', () => {
      expect(
        mountWithRootTheme(<NavigationItemGroup hasSeparator />)
        .find(NavigationItemGroupSeparator).length
      ).toBe(1);
    });
    it('with no action specified, no action should be rendered', () => {
      expect(
        mountWithRootTheme(<NavigationItemGroup />)
        .find(NavigationItemGroupAction).length
      ).toBe(0);
    });
