...
describe(name, () => {
  ...
  describe('values property', () => {
    it('should render the correct number of indicators', () => {
      const wrapper = shallow(<ProgressDots selectedIndex={0} values={['one', 'two', 'three', 'four', 'five']} />);
      expect(wrapper.find(IndicatorDiv)).toHaveLength(5);
    });
    it('should accept an array of any value types', () => {
      const numericValues = shallow(<ProgressDots selectedIndex={0} values={[1, 2, 3]} />);
      const objectValues = shallow(<ProgressDots selectedIndex={0} values={[{ key: 'value' }, { key: 'value' }, { key: 'value' }]} />);
