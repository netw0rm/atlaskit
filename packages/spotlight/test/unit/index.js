...
import Spotlight, { SpotlightStateless } from '../../src';
...
describe(name, () => {
  describe('stateless', () => {
    it('should print out when it is active', () => {
      const wrapper = mount(<SpotlightStateless isActive />);
      expect(wrapper.find('button').text()).toBe('active');
    });

    it('should print out when it is not active', () => {
      const wrapper = mount(<SpotlightStateless />);
    ...
    it('should fire onSpotlightStateless when the button is clicked', () => {
      const stub = jest.fn();
      const wrapper = mount(
        <SpotlightStateless
          onSpotlightStateless={stub}
        />
      );
  ...
  describe('stateful', () => {
    it('should toggle the active state of the SpotlightStateless onSpotlightStateless', () => {
      const wrapper = mount(<Spotlight label="hello" />);
      const stateless = wrapper.find(SpotlightStateless);

      expect(stateless.props().isActive).toBe(false);

      stateless.props().onSpotlightStateless();
