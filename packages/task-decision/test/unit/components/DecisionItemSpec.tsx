...
describe('<DecisionItem/>', () => {
  ...
  describe('attribution', () => {
    ...
    it('Creator, no updater', () => {
      const component = mount(<DecisionItem appearance="card" creator={user1} />);
      const attributionWrapper = component.find(AttributionWrapper);
      expect(attributionWrapper.length).toEqual(1);
      expect(attributionWrapper.at(0).text()).toEqual(`Captured by ${user1.displayName}`);
    });

    it('Creator and lastUpdater', () => {
      const component = mount(<DecisionItem appearance="card" creator={user1} lastUpdater={user2} />);
      const attributionWrapper = component.find(AttributionWrapper);
      expect(attributionWrapper.length).toEqual(1);
      expect(attributionWrapper.at(0).text()).toEqual(`Captured by ${user2.displayName}`);
    });

    it('Creator and lastUpdater, done, inline - no attribution', () => {
      const component = mount(<DecisionItem appearance="inline" creator={user1} lastUpdater={user2} />);
      const attributionWrapper = component.find(AttributionWrapper);
