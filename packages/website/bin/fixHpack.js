...
/*
  ...
  the website.
   ...
   just add it ourself.
  We dont know if a user has installed this package using npm/yarn in the package, or if it uses bootstrap
  from the root, so we have to check a couple of locations (and we'll fix any that we find)
*/

const nodeModulesPath = path.join(__dirname, '..', 'node_modules');
// The location of the hpack package.json file if it has been flattened (when yarn installing for example)
const hpackFlatInstallPath = path.join(nodeModulesPath, 'hpack.js', 'package.json');
// The location of the hpack package.json file if it is installed 'global-style' (i.e, when bootstrapped from root)
