...
test('ConfirmTrial should fire an appropriate analytics event when it is mounted', () => {
  ...
  mount(
      withAnalyticsSpy(
      spy,
        <ConfirmTrialBase {...defaultProps} />
      )
    );
  return waitFor(() => expect(spy).toHaveBeenCalledWith('xflow.confirm-trial.displayed', expect.any(Object)));
});
...
test('ConfirmTrial should fire an appropriate analytics event when the confirm button is clicked', () => {
  ...
  return waitFor(() => expect(spy).toHaveBeenCalledWith('xflow.confirm-trial.confirm-button.clicked', expect.any(Object)));
});
...
test('ConfirmTrial should fire an appropriate analytics event when the cancel button is clicked', () => {
  ...
  return waitFor(() => expect(spy).toHaveBeenCalledWith('xflow.confirm-trial.cancel-button.clicked', expect.any(Object)));
});
